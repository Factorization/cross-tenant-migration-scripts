<#
 ...
 .Parameter InputFile
	The path to the file containing the users you want to export.

 .Parameter DomainName
	The AD domain name. This is used for the Get-ADUser command. This can also be a specific Domain controller.

 .Example
	.\CrossTenantObjectExport.ps1 -InputFile "Migration_Users.txt" -DomainName ad.domain

	This runs the basic user export for the users that are in the file Migration_Users.txt.

 .Example
	.\CrossTenantObjectExport.ps1 -InputFile "Migration_Users.txt" -DomainName ad.domain -IncludeInboxRules

	This runs the basic export and includes the inbox rules for each user (This requires Exchange Admin)

 .Example
	.\CrossTenantObjectExport.ps1 -InputFile "Migration_Users.txt" -DomainName ad.domain -IncludeMailboxStats

	This runs the basic export and includes the mailbox and folder statistics for each user (This requires Exchange Admin)

 .Example
	.\CrossTenantObjectExport.ps1 -InputFile "Migration_Users.txt" -DomainName ad.domain -IncludeMailboxPermissions

	This runs the basic export and includes the mailbox permissions for each user (This requires Exchange Admin)

 .Example
	.\CrossTenantObjectExport.ps1 -InputFile "Migration_Users.txt" -DomainName ad.domain -IncludeInboxRules -IncludeMailboxStats -IncludeMailboxPermissions
...
BEGIN {
	...
	# Get Users
	Try {
		Write-Verbose "Importing users from file $InputFile..."
		$TextFile = Get-Content $InputFile
	}
	Catch {
		Write-Host "Input file  `"$InputFile`" does not exist. Please verify the file and try again." -ForegroundColor Red
		Break
	}
	Write-Verbose "Getting users from $InputFile..."
	$Members = $TextFile | ForEach-Object {Get-EXORecipient $_}
	if (-not $Members) {
		Write-Host "No users found from input file $InputFile. Please verify the input file and try again."
		Break
	}
